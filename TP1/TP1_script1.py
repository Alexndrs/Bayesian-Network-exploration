...
print("P(G) = ", P_G)
print("P(G) = ", P_G_)
print("We can see the two methods give the same result")

print("\n\n----- b) P(G|R=r1) -----\n")

# b) P(G|R=r1)
#    P(G|R=r1) = \sum_{s,d,l} P(S=s).P(D=d).P(R=r1|S=s).P(G|S=s,D=d).P(L=l|G)
# def compute_P_G_R1():
#     P_G_R1 = np.zeros(3)
#     for s in range(2):
#         for d in range(2):
#             for l in range(2):
#                 P_G_R += P_S[s] * P_D[d] * P_R_S[1,s] * P_G_SD[s,d] * P_L_G[l]
#     return P_G_R1

# P_G_R1 = compute_P_G_R1()
# print("P(G|R=r1) = ", P_G_R1)